...
def Preprocesa_TomateFermentado2(archivo):
    try:
        df=archivo
        ...
        columnas_a_eliminar = [col for col in df.columns if col not in ['Sample', 'R(BME680)', 'H2(SGP30)', 'SRAW(SGP40)', 'Ethanol(SGP30)', 'Air/Sample','tOUT_PID-AH2']]
        ...
        ciclos_tiempo_valor_mínimo = []             # Lista para almacenar el tiempo en el que se produce el valor mínimo de cada ciclo
        ...
        #Si existe la columna 'tOUT_PID-AH2', la multplicamos por -1.
        if 'tOUT_PID-AH2' in df.columns:
            df['tOUT_PID-AH2'] = df['tOUT_PID-AH2'] * -1
        ...
        for i in range(len(ciclos)):
            ...
            #Recorremos cada columna del ciclo y guardamos el tiempo en el que se produce el valor mínimo en un dataframe aparte.
            ciclos_tiempo_valor_mínimo.append(ciclos[i].idxmin().to_frame().T)  # Convertimos la Serie en un DataFrame de una sola fila
        ...
        # Hacemos lo mismo con los dataframes de tiempo de tiempo_valor_mínimo,delta y delta relativo.
        ciclos_tiempo_valor_mínimo = pd.concat(ciclos_tiempo_valor_mínimo, ignore_index=True)
        ciclos_tiempo_valor_mínimo.columns = [col + '_min_time' for col in ciclos_tiempo_valor_mínimo.columns]
        ...
        ciclos_valores_característicos = pd.concat([ciclos_valores_mínimos, ciclos_delta,ciclos_delta_rel,ciclos_tiempo_valor_mínimo], axis=1)  # Concatenamos los dataframes de valores mínimos y delta
        ...
        logging.error(f"Error while preprocessing file: {e}")
