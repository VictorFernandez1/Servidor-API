...
def preprocesa(archivo):
    try:
        ...
        if int(df.iloc[-1, 0]) != int(num_ciclos)*(int(tamaño_ciclo)):
            ...
            logging.info(f"En el archivo {archivo} el número de filas es {int(df.iloc[-1, 0])} y debería ser {int(num_ciclos)*(int(tamaño_ciclo))}. Parece que el archivo sufrió perdida de conexión. ¿Desea preprocesarlo igualmente?")
        ...
        logging.error(f"Error al preprocesar el archivo {archivo}: {e}")
